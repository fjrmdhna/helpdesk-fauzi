...
class AdminTicketController extends Controller
{
    ...
    private function emailpassgueststore($request)
    {
        ...
        $ticket->update();
        app(TicketAutoAssignmentService::class)->assignToDefaultAdmin($ticket);
    ...
    public function adminalltickets()
    {
        ...
        $alltickets = Ticket::with('ticket_customfield')->latest('updated_at')->get();
    ...
    public function exportTickets(Request $request)
    {
        ...
        // If the user is not a superadmin or employee with no group, apply group filtering
        if(Auth::user()->getRoleNames()[0] != 'superadmin') {
            $groupexists = Groupsusers::where('users_id', Auth::id())->exists();
            
            if($groupexists) {
                // Get user's groups
                $groups = Groupsusers::where('users_id', Auth::id())->pluck('groups_id');
                // Get categories assigned to these groups
                $categories = DB::table('groups_categories')
        ...
        foreach($tickets as $ticket) {
            $data[] = [
                ...
                $ticket->conversationExportText(Auth::user()->timezone ?? setting('default_timezone')),
                $ticket->status,
                $ticket->purchasecodesupport ?? 'N/A',
                $ticket->cust->email ?? 'N/A',
                $ticket->lastReplyExportText(Auth::user()->timezone ?? setting('default_timezone'))
            ];
        }
