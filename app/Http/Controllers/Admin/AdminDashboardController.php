...
class AdminDashboardController extends Controller
{
    ...
    public function adminDashboard()
    {
        ...
        // Global ticket stat cards (totals + on-hold; same layout as customer dashboard)
        $data = array_merge($data, $this->globalTicketMetricsForDashboard());

        $recentticketlist = Ticket::where('status','New')->with('ticket_customfield')->get();
        $recentticketcount = 0;
        ...
        $myassignedticket = Ticket::leftJoin('ticketassignchildren','ticketassignchildren.ticket_id','tickets.id')
            ->where('toassignuser_id',Auth::id())
            ->where('status', '!=' ,'Closed')
            ->where('status', '!=' ,'Suspend')
            ->with('ticket_customfield')
            ->get();
        ...
        $alltickets = Ticket::whereIn('status', ['New'])->with('ticket_customfield')->latest('updated_at')->get();
        $data['alltickets'] = $alltickets;

        $data['gtickets'] = Ticket::whereIn('status', ['New'])->with('ticket_customfield')->latest('updated_at')->get();
    ...
    public function Dashboard()
    {
        ...
        if($groupexists){
            ...
            ->where(function($q){
                $q->where(function($q2){
                    $q2->whereNull('tickets.myassignuser_id')
                       ->whereNull('tickets.selfassignuser_id');
                })->orWhereHas('ticketassignmutliples', function($q3){
                    $q3->where('toassignuser_id', Auth::id());
                });
            })
            ->distinct('tickets.id')
            ->count('tickets.id');
        ...
        // If no there in group we get the all tickets
            ...
            $totalactivetickets = Ticket::where('status', '!=', 'Closed')
                ->where(function($q){
                    $q->where(function($q2){
                        $q2->whereNull('myassignuser_id')
                           ->whereNull('selfassignuser_id');
                    })->orWhereHas('ticketassignmutliples', function($q3){
                        $q3->where('toassignuser_id', Auth::id());
                    });
                })
            ...
            $totalactiverecent = Ticket::where('status', '!=', 'Closed')
                ->where('replystatus', 'Replied')
                ->where(function($q){
                    $q->where(function($q2){
                        $q2->whereNull('myassignuser_id')
            ...
            $recentticketcount = Ticket::where('status', 'New')
                ->where(function($q){
                    $q->where(function($q2){
                        $q2->whereNull('myassignuser_id')
                           ->whereNull('selfassignuser_id');
    ...
    {
        $data['alltickets'] = Ticket::whereIn('status', ['New'])
            ->with([
                'ticket_customfield',
                'cust',
    ...
    public function Dashboardtabledatas()
    {
        ...
        if($groupexists){
            ...
            $gticket = Ticket::select('tickets.*',"groups_categories.group_id","groups_users.users_id")
                ...
                ->whereNotNull('groups_users.users_id')
                ->where('groups_users.users_id', Auth::id())
                ->with(['ticket_customfield', 'ticketassignmutliples', 'cust', 'ticketnote'])
    ...
    {
        $allactivetickets = Ticket::active()->with('ticket_customfield')->latest('updated_at')->get();
        ...
        $allactiveassignedtickets = Ticket::active()->where(function($r){
            $r->whereNotNull('myassignuser_id')
            ->orWhereNotNull('selfassignuser_id');
        })->count();
        $data['allactiveassignedtickets'] = $allactiveassignedtickets;

        $allactiveoverduetickets = Ticket::active()->whereNotNull('overduestatus')->count();
    ...
    public function closedticket()
    {
        ...
        $allclosedtickets = Ticket::where('status', 'Closed')->with('ticket_customfield')->latest('updated_at')->get();
    ...
    public function assignedTickets()
    {
        ...
        $assignedtickets = Ticket::where('myassignuser_id', Auth::id())
            ->whereNull('selfassignuser_id')
            ->where('status', '!=' ,'Closed')
            ->with('ticket_customfield')
            ->latest('updated_at')
    ...
    public function myassignedTickets()
    {
        ...
        $myassignedtickets = Ticket::select('tickets.*', 'ticketassignchildren.toassignuser_id')
            ->whereNull('selfassignuser_id')
    ...
    {
        $allonholdtickets = Ticket::where('status','On-Hold')->with('ticket_customfield')->latest('updated_at')->get();
    ...
    {
        $alloverduetickets = Ticket::whereIn('overduestatus', ['Overdue'])->with('ticket_customfield')->latest('updated_at')->get();
    ...
    public function employeeoverdueticket()
    {
        ...
        $tickets = Ticket::whereIn('overduestatus', ['Overdue'])->with('ticket_customfield')->get();
    ...
    {
        $allassignedtickets = Ticket::with('ticket_customfield')->latest('updated_at')->get();
    ...
    {
        $recenttickets = Ticket::whereIn('status', ['New'])
            ->with(['ticket_customfield', 'ticketassignmutliples'])
            ->latest('updated_at')
            ->get();
   ...
   {
        $data['suspendedtickets'] = Ticket::where('status', 'Suspend')->with('ticket_customfield')->latest('updated_at')->get();
    ...
    /**
     * Counters for dashboard "Global Tickets" cards (aligned with customer dashboard metrics).
     * Superadmin: all tickets. Helpdesk employees in groups: tickets in assigned categories only.
     */
    protected function globalTicketMetricsForDashboard(): array
    {
        if (Auth::user()->dashboard === 'Admin') {
            return [
                'totaltickets' => Ticket::count(),
                'totalactivetickets' => Ticket::active()->count(),
                'totalonholdtickets' => Ticket::where('status', 'On-Hold')->count(),
                'totalclosedtickets' => Ticket::where('status', 'Closed')->count(),
                'replyrecent' => Ticket::active()->where('replystatus', 'Replied')->count(),
            ];
        }
        ...
        $groupIdsString = substr($group_id, 0, -1);
        $groupIds = $groupIdsString !== '' ? array_filter(explode(',', $groupIdsString)) : [];

        $groupexists = $groupIds !== []
            && Groupsusers::whereIn('groups_id', $groupIds)->where('users_id', Auth::id())->exists();

        if ($groupexists) {
            $categoryIds = DB::table('groups_categories')
                ->join('groups_users', 'groups_users.groups_id', '=', 'groups_categories.group_id')
                ->where('groups_users.users_id', Auth::id())
                ->distinct()
                ->pluck('groups_categories.category_id')
                ->filter()
                ->unique()
                ->values()
                ->all();
            ...
            $base = Ticket::whereIn('category_id', $categoryIds);

            return [
                'totaltickets' => (clone $base)->count(),
                'totalactivetickets' => (clone $base)->where('status', '!=', 'Closed')->count(),
                'totalonholdtickets' => (clone $base)->where('status', 'On-Hold')->count(),
                'totalclosedtickets' => (clone $base)->where('status', 'Closed')->count(),
                'replyrecent' => (clone $base)->where('status', '!=', 'Closed')->where('replystatus', 'Replied')->count(),
            ];
        }
