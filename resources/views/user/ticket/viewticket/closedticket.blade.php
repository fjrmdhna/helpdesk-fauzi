                            ...
                            <div class="table-responsive">
                                @if(setting('CUSTOMER_RESTICT_TO_DELETE_TICKET') == 'off')
                                    <div class="d-flex align-items-center mb-4 gap-2 data-table-btn">
                                        <button id="massdelete" class="btn btn-outline-light btn-sm" data-bs-toggle="tooltip" data-bs-placement="top" title="{{lang('Delete')}}"><i class="fe fe-trash me-1"></i></button>
                                        <button id="exportExcel" class="btn btn-outline-success btn-sm" data-bs-toggle="tooltip" data-bs-placement="top" title="{{lang('Export to Excel')}}"><i class="fe fe-download me-1"></i></button>
                                    </div>
                                @else
                                    <div class="d-flex align-items-center mb-4 gap-2 data-table-btn">
                                        <button id="exportExcel" class="btn btn-outline-success btn-sm" data-bs-toggle="tooltip" data-bs-placement="top" title="{{lang('Export to Excel')}}"><i class="fe fe-download me-1"></i></button>
                                ...
                                <table
                                    ...
                                    <tbody>
                                        ...
                                        <tr {{$closedticket->replystatus == 'Waiting'? 'class=bg-success-transparent': ''}}>
                                            ...
                                            <td>
                                                    {{$closedticket->created_at->timezone(Auth::user()->timezone)->format(setting('date_format'))}}
                                                </td>
                                                <td>
                                                    @if($closedticket->closing_ticket != null)
                                                        {{$closedticket->closing_ticket->timezone(Auth::user()->timezone)->format(setting('date_format'))}}
                                                    @else
                                                        <span class="text-muted">{{lang('Open')}}</span>
                                                    @endif
                                                </td>
                                                <td>
                                                    @php
                                                        $endDate = $closedticket->closing_ticket ?? now();
                                                        $duration = $closedticket->created_at->diffInHours($endDate);
                                                ...
                                                <td class="text-muted">
                                                    {{ $closedticket->customFieldValue('Location') ?? '~' }}
                                                </td>
                                                <td class="text-muted">
                                                    {{ $closedticket->customFieldValue('EOS Indication') ?? '~' }}
                                                </td>
                                                <td class="text-muted">
                                                    {{ $closedticket->lastReplyText(90) ?? '~' }}
                                                </td>
                                                <td>
...
{{-- export modal is included globally in `layouts.usermaster` --}}
