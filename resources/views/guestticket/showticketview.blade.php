...
@extends('layouts.usermaster')
										...
										<div class="table-responsive tr-lastchild">
											<table class="table mb-0 table-information">
												<tbody>
													...
													@php $closedReasonLabel = $ticket->closedReasonLabel(); @endphp
													@if($ticket->status === 'Closed' && $closedReasonLabel)
														<tr>
															<td>
																<span class="w-50">{{ lang('Closed Reason') }}</span>
															</td>
															<td>:</td>
															<td>
																<span class="font-weight-semibold">{{ lang($closedReasonLabel) }}</span>
															</td>
														</tr>
													@endif
																	...
																	<div class="form-group">
																		<div class="custom-controls-stacked d-md-flex">
																			...
																			<label class="custom-control form-radio success">
																				<input type="radio" class="custom-control-input" name="status" value="Closed" @if(old('status') === 'Closed') checked @endif>
																				<span class="custom-control-label">{{lang('Solved')}}</span>
																			</label>
																		</div>
																	</div>
                                                                    <div class="form-group mt-3 js-closed-reason-wrapper {{ old('status') === 'Closed' ? '' : 'd-none' }}" data-trigger-status="Closed">
                                                                        <label class="form-label">{{ lang('Closed Reason') }}</label>
                                                                        <div class="custom-controls-stacked d-md-flex">
                                                                            <label class="custom-control form-radio success me-4">
                                                                                <input type="radio" class="custom-control-input js-closed-reason-input @error('closed_reason') is-invalid @enderror" name="closed_reason" value="by_customer" @if(old('closed_reason') === 'by_customer') checked @endif>
                                                                                <span class="custom-control-label">{{ lang('By Customer') }}</span>
                                                                            </label>
                                                                            <label class="custom-control form-radio success me-4">
                                                                                <input type="radio" class="custom-control-input js-closed-reason-input @error('closed_reason') is-invalid @enderror" name="closed_reason" value="by_supplier" @if(old('closed_reason') === 'by_supplier') checked @endif>
                                                                                <span class="custom-control-label">{{ lang('By Supplier') }}</span>
                                                                            </label>
                                                                        </div>
                                                                        @error('closed_reason')
		...
		<script type="text/javascript">
            ...
            document.querySelectorAll('.js-closed-reason-wrapper').forEach((wrapper) => {
                const form = wrapper.closest('form');
                if (!form) {
                    return;
                }
                const triggerStatus = wrapper.dataset.triggerStatus || 'Closed';
                const radios = form.querySelectorAll('input[type="radio"][name="status"]');
                const reasonInputs = wrapper.querySelectorAll('.js-closed-reason-input');
