@extends('layouts.adminmaster')
									...
									<div class="card-body overflow-scroll" >
										<div class=" spruko-delete">
											<div class="d-flex align-items-center mb-4 gap-2 data-table-btn">
												@can('Ticket Delete')
												<button id="massdelete" class="btn btn-outline-light btn-sm" data-bs-toggle="tooltip" data-bs-placement="top" title="{{lang('Delete')}}"><i class="fe fe-trash"></i></button>
												@endcan
												<button id="exportExcel" class="btn btn-outline-success btn-sm" data-bs-toggle="tooltip" data-bs-placement="top" title="{{lang('Export to Excel')}}"><i class="fe fe-download"></i></button>
											</div>
											<div>
												<table class="table table-bordered border-bottom text-nowrap"  id="overduetickets">
													<thead >
														<tr >
															...
															@include('admin.partials.ticket-list-datetime-head')
															<th>{{lang('Location')}}</th>
															<th>{{lang('EOS Indication')}}</th>
													...
													<tbody id="refresh">
														...
														@php
															$assignedIds = $tickets->ticketassignmutliples?->pluck('toassignuser_id')->toArray() ?? [];
															$inAssigned = Auth::check() && in_array(Auth::id(), $assignedIds);
														@endphp
														@if(($tickets->myassignuser_id == null && $tickets->selfassignuser_id == null) || $inAssigned)
														...
														<tr {{$tickets->replystatus == 'Replied'? 'class=bg-success-transparent': ''}}>
																...
																<div class="d-flex align-items-center">
																	<div class="">
																		...
																		<ul class="fs-13 font-weight-normal d-flex custom-ul">
																			<li class="pe-2 text-muted">#{{$tickets->ticket_id}}</li>
															...
															@include('admin.partials.ticket-list-datetime-cells', ['ticket' => $tickets])
															<td class="text-muted">
																{{ $tickets->customFieldValue('Location') ?? '~' }}
															</td>
															<td class="text-muted">
																{{ $tickets->customFieldValue('EOS Indication') ?? '~' }}
															</td>
															<td class="text-muted">
																{{ $tickets->lastReplyText(90) ?? '~' }}
															</td>
															<td>
		...
		@endsection
			...
			{{-- export modal is included globally in `layouts.adminmaster` --}}
			@endsection
